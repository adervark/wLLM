...
3. **Structured Output (JSON mode)**: OpenAI-style `response_format` with grammar-constrained decoding via xgrammar — guaranteed-valid JSON from any model, even zero-day ones (`pip install winllm[structured]`).
4. **CUDA Graph Decode**: `--cuda-graphs` captures the decode forward in a CUDA graph (no Triton/torch.compile needed on Windows) — up to ~7× single-request decode throughput, with an automatic eager fallback for graph-unsafe architectures.
5. **Optimized Speculative Decoding**: Integrated draft models accelerate inference by predicting multiple tokens per target forward pass. `--suffix-decoding` enables a model-free variant (SuffixDecoding, NeurIPS 2025) that drafts from the request's own prompt/output — no draft model, no extra VRAM, output guaranteed token-identical, with an automatic load-time probe that disables it on architectures whose caches can't roll back losslessly.
6. **Optional Native Accelerators**: opt-in C++/CUDA modules under `native/` (a fused sampling kernel that collapses the per-token sampling pipeline into one launch, a grammar-bitmask kernel, and a C++ suffix cache) — build them once with `uv pip install ./native/sampling ./native/suffix` if you have MSVC + the CUDA toolkit, or don't: wLLM detects their absence and runs identically on the pure-torch paths.
7. **Multi-Backend Choice**: Native support for **PyTorch**, **ONNX Runtime (via Optimum)**, and **DirectML**.
8. **Production Observability**: Prometheus `/metrics` endpoint with TTFT / latency / throughput percentiles.
...
**Native accelerators (optional, needs MSVC Build Tools; `native/sampling` also needs the CUDA toolkit):**
```bash
uv pip install ./native/suffix      # C++ suffix cache for --suffix-decoding
uv pip install ./native/sampling    # fused CUDA sampling + grammar bitmask kernels
```
Skip this entirely if you don't have the compilers — everything falls back to the pure-torch implementations automatically.
...
- [SOLID Architecture](documentation/SOLID_Architecture.md) — Package layout and the design principles behind it.
- [Legacy Code Problems](documentation/Legacy_Code_Problems.md) — What was wrong with the old flat layout and why it was restructured.
