...
def configure_cuda_backends() -> None:
    ...
    Called once before model loading. These are global PyTorch settings:
      ...
      - Expandable segments: Reduces CUDA memory fragmentation. Linux-only:
        torch warns and ignores the setting on Windows.
    """
    import torch
    ...
    # Unsupported on Windows (torch emits a UserWarning and ignores it).
    alloc_conf = os.environ.get('PYTORCH_CUDA_ALLOC_CONF', '')
    if sys.platform != 'win32' and 'expandable_segments' not in alloc_conf:
