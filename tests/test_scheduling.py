...
import time
import pytest
from winllm.kvcache.prefix import compute_prefix_hashes
from winllm.scheduling import SchedulerStats
...
class TestGetPrefixHashes:
    def test_empty_tokens(self):
        assert compute_prefix_hashes([], block_size=16) == []

    def test_shorter_than_block_size(self):
        """Tokens shorter than one block should produce no hashes."""
        hashes = compute_prefix_hashes([1, 2, 3], block_size=16)
        assert hashes == []

    def test_exactly_one_block(self):
        tokens = list(range(16))
        hashes = compute_prefix_hashes(tokens, block_size=16)
        assert len(hashes) == 1
    ...
    def test_two_blocks(self):
        ...
        one_block = compute_prefix_hashes(tokens[:16], block_size=16)
    ...
    def test_deterministic(self):
        ...
        tokens = list(range(32))
        h1 = compute_prefix_hashes(tokens, block_size=16)
        h2 = compute_prefix_hashes(tokens, block_size=16)
        assert h1 == h2

    def test_different_tokens_different_hashes(self):
        t1 = list(range(16))
        t2 = list(range(100, 116))
        h1 = compute_prefix_hashes(t1, block_size=16)
        h2 = compute_prefix_hashes(t2, block_size=16)
        assert h1 != h2

    def test_block_size_1(self):
        tokens = [10, 20, 30]
        hashes = compute_prefix_hashes(tokens, block_size=1)
        assert len(hashes) == 3
...
class TestPrefixHashConsistency:
    def test_same_prefix_same_hash(self):
        ...
        h1 = compute_prefix_hashes(tokens1, block_size=16)
        h2 = compute_prefix_hashes(tokens2, block_size=16)
    ...
    def test_hash_is_cumulative(self):
        ...
        hashes_modified = compute_prefix_hashes(tokens_modified, block_size=16)
    ...
    def test_large_block_size(self):
        ...
        hashes = compute_prefix_hashes(tokens, block_size=512)
        assert len(hashes) == 2
