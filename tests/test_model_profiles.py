...
from winllm.models.profiles import identify_model_profile
...
def test_model_config_auto_registry():
    ...
    from winllm.hardware import HardwareDefaults
    from winllm.hardware.tuning import apply_model_defaults
    ...
    # Base instantiation
    assert config.quantization == QuantizationType.AUTO
    assert config.max_model_len == 4096

    # Apply defaults
    apply_model_defaults(config, mock_hw_defaults)

    # HW default changes quant from AUTO -> NONE.
    # Registry detects "qwen" but apply_model_profile only overrides if quant is still AUTO.
