...
| Command | Description |
|---|---|
| [`serve`](#wllm-serve) | Start an OpenAI-compatible API server ([source](../winllm/cli/commands/serve.py)) |
| [`chat`](#wllm-chat) | Interactive chat session in the terminal ([source](../winllm/cli/commands/chat.py)) |
| [`benchmark`](#wllm-benchmark) | Run a throughput benchmark ([source](../winllm/cli/commands/benchmark.py)) |
| [`list`](#wllm-list) | List downloaded models from HuggingFace cache ([source](../winllm/cli/commands/list.py)) |
| [`detect`](#wllm-detect) | Detect and display hardware info ([source](../winllm/cli/commands/detect.py)) |
| [`remove`](#wllm-remove) | Remove downloaded models from cache ([source](../winllm/cli/commands/remove.py)) |

---

## `wllm serve`

Start the API server with OpenAI-compatible endpoints (`/v1/chat/completions`, `/v1/completions`, `/v1/models`, `/health`) plus a Prometheus `/metrics` endpoint (request/token counters, queue and KV-cache gauges, p50/p90/p99 TTFT, latency, and throughput).

Chat completions accept OpenAI-style structured output via `response_format` (`{"type": "json_object"}` or `{"type": "json_schema", "json_schema": {"schema": {...}}}`); requires the optional xgrammar dependency (`pip install winllm[structured]`).
...
| `--cuda-graphs` | | `false` | Capture single-request decode in a CUDA graph (experimental; auto-falls back if the model isn't graph-safe) |
| `--suffix-decoding` | | `false` | Model-free speculative decoding via suffix matching (no draft model; auto-disabled if the model cache cannot roll back losslessly) |
...
# CUDA graph decode for maximum single-request throughput
wllm serve -m HuggingFaceTB/SmolLM2-135M-Instruct -q none --cuda-graphs

# Model-free speculative decoding (no draft model needed; biggest wins on
# repetitive/structured/agentic output — output stays token-identical)
wllm serve -m HuggingFaceTB/SmolLM2-135M-Instruct -q none --suffix-decoding
```
