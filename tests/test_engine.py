...
class TestEngineLifecycle:
    ...
    @patch("winllm.inference.engine.ModelLoader")
...
class TestValidation:
    def test_validate_prompt_valid(self, engine):
        ...
        assert engine._blocking_generator.validate_prompt(req, 100) is True
        assert req.status != RequestStatus.FAILED

    def test_validate_prompt_too_long(self, engine):
        engine.model_config.max_model_len = 100
        req = GenerationRequest()
        assert engine._blocking_generator.validate_prompt(req, 100) is False
        assert req.status == RequestStatus.FAILED
...
class TestKVCacheAllocation:
    def test_allocate_success(self, engine):
        ...
        req = GenerationRequest()
        result = engine._blocking_generator.allocate_kv_cache(req, 100, 50)
...
class TestMakeGenerator:
    def test_no_seed_returns_none(self, engine):
        ...
        assert engine._blocking_generator.make_generator(params, torch.device("cpu")) is None

    def test_with_seed_returns_generator(self, engine):
        params = SamplingParams(seed=42)
        gen = engine._blocking_generator.make_generator(params, torch.device("cpu"))
...
class TestStopConditions:
    def test_with_eos_token(self, engine):
        ...
        stop_ids, stop_strings = engine._blocking_generator.get_stop_conditions(params)
    ...
    def test_with_stop_strings(self, engine):
        ...
        _, stop_strings = engine._blocking_generator.get_stop_conditions(params)
...
class TestEmitStreamToken:
    def test_token_callback_path(self, engine):
        """_token_callback drains every not-yet-emitted token ID in order.

        Draining (rather than sending only the last token) is what keeps
        streaming lossless when speculative decoding commits several tokens
        in one step.
        """
        received = []
        ...
        engine._emitter.emit(req)
        assert received == [(10, False), (20, False), (30, False)]

        # Cursor advances: nothing is re-emitted
        engine._emitter.emit(req)
    ...
    def test_no_callback_noop(self, engine):
        ...
        engine._emitter.emit(req)  # Should not raise
...
class TestResolveDevice:
    def test_resolve_from_model_params(self, engine):
        ...
        device = engine._runtime.resolve_device()
...
class TestRequestCompletionStates:
    ...
    def test_stream_finished_signal(self, engine):
        """emit drains pending token IDs, ending with the newest one."""
        received = []
        ...
        assert received == [(42, False), (99, False)]
